...
let content = fs.readFileSync('lib/countryFilterData.ts', 'utf-8');
const match = content.match(/export const COUNTRY_FILTER_DATA: Record<string, CountryFilterMeta> = ({[\s\S]+?});/);
if (match) {
  let data;
  eval('data = ' + match[1]);
  ...
  Object.keys(data).forEach(slug => {
    ...
    if (slug === 'south-korea') mappedSlug = 'south-korea';
    if (slug === 'costa-rica') mappedSlug = 'costa-rica';
    if (slug === 'sri-lanka') mappedSlug = 'sri-lanka';
    if (slug === 'saudi-arabia') mappedSlug = 'saudi-arabia';
    if (slug === 'turkey') mappedSlug = 'türkiye';
    if (slug === 'czech-republic') mappedSlug = 'czechia';
    
    if (speedMap.has(mappedSlug)) {
      data[slug].internetMbps = speedMap.get(mappedSlug);
    } else {
  ...
  let output = 'export type CountryFilterMeta = {\n' +
    ...
    '};\n\n' +
    'export const COUNTRY_FILTER_DATA: Record<string, CountryFilterMeta> = ' + JSON.stringify(data, null, 2) + ';\n';
