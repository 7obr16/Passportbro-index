export type CountryFilterMeta = {
  receptiveness: string;
  localValues: string;
  englishProficiency: string;
  budgetTier: string;
  visaEase: string;
  internetSpeed: string;
  internetMbps?: number;
  climate: string;
  vibe: string;
  safetyLevel: string;
  healthcareQuality: string;
};

export const DEFAULT_COUNTRY_FILTER_META: CountryFilterMeta = {
  receptiveness: "Medium",
  localValues: "Mixed",
  englishProficiency: "Moderate",
  budgetTier: "$1k-$2k",
  visaEase: "Visa-Free",
  internetSpeed: "Moderate",
  climate: "Temperate",
  vibe: "",
  safetyLevel: "Moderate",
  healthcareQuality: "Moderate",
};

export const COUNTRY_FILTER_DATA: Record<string, CountryFilterMeta> = {
  philippines: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Beach Access, Nature/Mountains, Great Nightlife",
    safetyLevel: "Dangerous",
    healthcareQuality: "Moderate",
    internetMbps: 103.91,
  },
  thailand: {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "High",
    internetMbps: 272.99,
  },
  indonesia: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Beach Access, Nature/Mountains, Great Nightlife",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 44.62,
  },
  malaysia: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 165.48,
  },
  vietnam: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "e-Visa",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "Great Nightlife, Nature/Mountains, Beach Access",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 284.99,
  },
  cambodia: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "<$1k",
    visaEase: "e-Visa",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
    internetMbps: 50.88,
  },
  kenya: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "e-Visa",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Nature/Mountains, Great Nightlife",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 50.63,
  },
  nigeria: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Difficult",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Great Nightlife",
    safetyLevel: "Dangerous",
    healthcareQuality: "Low",
    internetMbps: 44.06,
  },
  uganda: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "<$1k",
    visaEase: "e-Visa",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
    internetMbps: 27.56,
  },
  rwanda: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "e-Visa",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 41.02,
  },
  tanzania: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "e-Visa",
    internetSpeed: "Slow",
    climate: "Tropical",
    vibe: "Nature/Mountains, Beach Access",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
    internetMbps: 20.07,
  },
  ethiopia: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "e-Visa",
    internetSpeed: "Slow",
    climate: "Tropical",
    vibe: "Nature/Mountains",
    safetyLevel: "Dangerous",
    healthcareQuality: "Low",
  },
  bolivia: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Slow",
    climate: "Temperate",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
    internetMbps: 10.12,
  },
  colombia: {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "Great Nightlife, Nature/Mountains, Beach Access",
    safetyLevel: "Dangerous",
    healthcareQuality: "Moderate",
    internetMbps: 208.11,
  },
  mexico: {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Dangerous",
    healthcareQuality: "Moderate",
    internetMbps: 93.19,
  },
  peru: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains, Beach Access",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 256.52,
  },
  venezuela: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "<$1k",
    visaEase: "Difficult",
    internetSpeed: "Slow",
    climate: "Tropical",
    vibe: "Beach Access, Nature/Mountains",
    safetyLevel: "Dangerous",
    healthcareQuality: "Low",
    internetMbps: 96.6,
  },
  "dominican-republic": {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Great Nightlife, Beach Access",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 73.07,
  },
  "costa-rica": {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "Nature/Mountains, Beach Access",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 161.77,
  },
  india: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "<$1k",
    visaEase: "e-Visa",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Nature/Mountains, Beach Access",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 61.67,
  },
  pakistan: {
    receptiveness: "Low",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "e-Visa",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Nature/Mountains",
    safetyLevel: "Dangerous",
    healthcareQuality: "Low",
    internetMbps: 18.06,
  },
  morocco: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Beach Access, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 59.49,
  },
  brazil: {
    receptiveness: "High",
    localValues: "Modern",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 222.3,
  },
  argentina: {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 113.33,
  },
  chile: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains, Beach Access",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 337.86,
  },
  china: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$2k-$3k",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 216.96,
  },
  mongolia: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Cold",
    vibe: "Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Low",
    internetMbps: 81.13,
  },
  "south-africa": {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Dangerous",
    healthcareQuality: "High",
    internetMbps: 68.7,
  },
  russia: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Great Nightlife",
    safetyLevel: "Dangerous",
    healthcareQuality: "Moderate",
    internetMbps: 91.34,
  },
  ukraine: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Great Nightlife",
    safetyLevel: "Dangerous",
    healthcareQuality: "Moderate",
    internetMbps: 91.38,
  },
  poland: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 214.57,
  },
  romania: {
    receptiveness: "High",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 268.82,
  },
  turkey: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "e-Visa",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Dangerous",
    healthcareQuality: "High",
    internetMbps: 75.34,
  },
  kazakhstan: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 87.17,
  },
  algeria: {
    receptiveness: "Low",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Difficult",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Beach Access",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
    internetMbps: 41.45,
  },
  libya: {
    receptiveness: "Low",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Difficult",
    internetSpeed: "Slow",
    climate: "Tropical",
    vibe: "Beach Access",
    safetyLevel: "Dangerous",
    healthcareQuality: "Low",
    internetMbps: 24.36,
  },
  usa: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "High",
    internetMbps: 306.15,
  },
  canada: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 274.35,
  },
  australia: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 158.62,
  },
  uk: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 166.28,
  },
  france: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "Moderate",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 348.02,
  },
  germany: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 102.67,
  },
  spain: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "Moderate",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 274.21,
  },
  italy: {
    receptiveness: "Low",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 113.34,
  },
  sweden: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 203.36,
  },
  japan: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 248.85,
  },
  "south-korea": {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "Moderate",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 247.37,
  },
  "saudi-arabia": {
    receptiveness: "Low",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "$3k+",
    visaEase: "e-Visa",
    internetSpeed: "Fast",
    climate: "Arid",
    vibe: "Great Nightlife",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 160.79,
  },
  egypt: {
    receptiveness: "Low",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "e-Visa",
    internetSpeed: "Moderate",
    climate: "Arid",
    vibe: "Beach Access",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 47.15,
  },
  iran: {
    receptiveness: "Low",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "<$1k",
    visaEase: "Difficult",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Nature/Mountains",
    safetyLevel: "Dangerous",
    healthcareQuality: "Moderate",
    internetMbps: 22.05,
  },
  // Additional European countries
  portugal: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Beach Access, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 154.22,
  },
  netherlands: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Very high",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 178.52,
  },
  belgium: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 148.33,
  },
  austria: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 94.12,
  },
  switzerland: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains, Great Nightlife",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 250.88,
  },
  norway: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Very high",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 178.45,
  },
  denmark: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Very high",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 206.67,
  },
  finland: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Very high",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 112.34,
  },
  ireland: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Very high",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 125.56,
  },
  greece: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Beach Access, Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 78.92,
  },
  "czech-republic": {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 93.45,
  },
  hungary: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 112.78,
  },
  croatia: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Beach Access, Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 85.34,
  },
  serbia: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 72.11,
  },
  bulgaria: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains, Beach Access",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 88.22,
  },
  slovakia: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains, Great Nightlife",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 95.67,
  },
  lithuania: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains, Beach Access",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 102.34,
  },
  latvia: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains, Beach Access",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 98.45,
  },
  estonia: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 115.78,
  },
  slovenia: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains, Great Nightlife",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 89.12,
  },
  luxembourg: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Very high",
    budgetTier: "$3k+",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Great Nightlife, Nature/Mountains",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 235.44,
  },
  malta: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Beach Access, Great Nightlife",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
    internetMbps: 118.92,
  },
  cyprus: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Beach Access, Great Nightlife",
    safetyLevel: "Safe",
    healthcareQuality: "High",
    internetMbps: 92.34,
  },
  montenegro: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Beach Access, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 78.45,
  },
  "north-macedonia": {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 55.22,
  },
  albania: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Beach Access, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
    internetMbps: 62.34,
  },
  "bosnia-and-herzegovina": {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "Low",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
    internetMbps: 48.92,
  },
  moldova: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
    internetMbps: 72.18,
  },
  taiwan: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "City, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
  },
  singapore: {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k-$5k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "City",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
  },
  laos: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "<$1k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
  },
  "sri-lanka": {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Beach Access, Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
  },
  ecuador: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
  },
  paraguay: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "City, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
  },
  uruguay: {
    receptiveness: "Medium",
    localValues: "Modern",
    englishProficiency: "Moderate",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Temperate",
    vibe: "Beach Access, City",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
  },
  panama: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "Moderate",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Fast",
    climate: "Tropical",
    vibe: "City, Beach Access",
    safetyLevel: "Safe",
    healthcareQuality: "High",
  },
  guatemala: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Nature/Mountains",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
  },
  cuba: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "<$1k",
    visaEase: "Difficult",
    internetSpeed: "Slow",
    climate: "Tropical",
    vibe: "Beach Access, Nightlife",
    safetyLevel: "Moderate",
    healthcareQuality: "Low",
  },
  jamaica: {
    receptiveness: "Medium",
    localValues: "Mixed",
    englishProficiency: "High",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Tropical",
    vibe: "Beach Access, Nightlife",
    safetyLevel: "Dangerous",
    healthcareQuality: "Moderate",
  },
  georgia: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "Moderate",
    budgetTier: "$1k-$2k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",
    climate: "Temperate",
    vibe: "City, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "Moderate",
  },
  "united-arab-emirates": {
    receptiveness: "Low",
    localValues: "Modern",
    englishProficiency: "High",
    budgetTier: "$3k-$5k",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Desert",
    vibe: "City, Great Nightlife",
    safetyLevel: "Very Safe",
    healthcareQuality: "High",
  },
  mauritius: {
    receptiveness: "High",
    localValues: "Traditional",
    englishProficiency: "High",
    budgetTier: "$2k-$3k",
    visaEase: "Visa-Free",
    internetSpeed: "Moderate",

    internetMbps: 59.56,
    climate: "Tropical",
    vibe: "Beach Access, Nature/Mountains",
    safetyLevel: "Safe",
    healthcareQuality: "High",
  },
  belarus: {
    receptiveness: "Medium",
    localValues: "Traditional",
    englishProficiency: "Low",
    budgetTier: "$1k-$2k",
    visaEase: "Difficult",
    internetSpeed: "Fast",
    climate: "Cold",
    vibe: "City",
    safetyLevel: "Moderate",
    healthcareQuality: "Moderate",
  },
};
